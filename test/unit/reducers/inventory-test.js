...
describe('insert reducer', () => {
    ...
    it('ads to inventory when empty', () => {
        ...
        expect(inventory.insert(List(),{type: types.ADD_TO_INVENTORY, object: expectedObject})).toEqualImmutable(List([expectedObject]))
    })

    it('ads to inventory when not empty', () => {
        const expectedObject1 = {someprop: Math.random()}
        const expectedObject2 = {someprop: Math.random()}
        expect(inventory.insert(List([expectedObject1]),{type: types.ADD_TO_INVENTORY, object: expectedObject2})).toEqual(List([expectedObject1, expectedObject2]))
    })
})